  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: currency, // 👈 AUD here
            product_data: {
              name: `${planName} Plan`,
            },
            unit_amount: Math.round(Number(amount) * 100), // AUD cents
          },
          quantity: 1,
        },
      ],
      ...
      success_url:
        success_url ||
        `${process.env.NEXT_PUBLIC_CLIENT_URL}/payment/success?session_id={CHECKOUT_SESSION_ID}`,

      cancel_url:
        cancel_url ||
        `${process.env.NEXT_PUBLIC_CLIENT_URL}/payment/cancel`,
    });
